...
namespace MatrixInverse
{
  ...
  {
    double x, y, z;
    x = v1[1][0] * v2[2][0] - v2[1][0] * v1[2][0];
    y = v1[0][0] * v2[2][0] - v2[0][0] * v1[2][0];
    z = v1[0][0] * v2[1][0] - v2[0][0] * v1[1][0];

    double[][] result =  new double[][]{new double[]{x}, new double[]{y}, new double[]{z}};


    return result;
  }

  public static double[][] MatrixSubstract(double[][] M1, double[][] M2){
    int n = M1.Length;
    int m = M1[0].Length;
    double[][] result = MatrixCreate(n,m);
      for (int i = 0; i < n; ++i)
        for (int j = 0; j < m; ++j)
  ...
  public static double[][] MatrixMultiply(double[][] M, double P){
    int n = M.Length;
    int m = M[0].Length;
    double[][] result = MatrixCreate(n,m);
      for (int i = 0; i < n; ++i)
